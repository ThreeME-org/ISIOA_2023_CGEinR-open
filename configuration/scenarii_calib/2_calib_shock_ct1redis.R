...
series <- c("GDP" ,"EMS_CI_CO2","EMS_MAT_CO2", "EMS_Y_CO2", "EMS_CH_CO2", "rco2tax_vol", "redis_ct_h" , "redis_ct_ls")   %>% tolower
...
shock_ch <- mutate(selection,
                   ...
                   redis_ct_h = 1  , redis_ct_ls = 1       
                   ) %>% select(year, rco2tax_vol, redis_ct_h, redis_ct_ls) 
